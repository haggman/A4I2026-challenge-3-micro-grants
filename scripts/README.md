...
It discovers BEA's Direct Requirements table id from the API rather than hardcoding one — BEA
does not publish a list of table ids and says so explicitly, so any literal id you find in a blog
post is a guess. If discovery fails it prints every table BEA offers and stops rather than
picking one.
